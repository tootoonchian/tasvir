...
void init(void) {
    ...
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<uint64_t> dist_offset(0x1000, UINT64_MAX);
...
void random_write(uint8_t *addr, size_t len) {
    size_t copy_len = 8;
    size_t offset = xorshift128plus() % (len - copy_len);
    addr += offset;
    memcpy(addr, &rand_arr[offset % RNDCNT], copy_len);
    if (do_log)
        tasvir_log_write(addr, copy_len);
}

uint64_t gettime_us() { return 1E6 * rte_rdtsc() / rte_get_tsc_hz(); }

uint64_t experiment(tasvir_area_desc *d, int count, int iter) {
...
int main(int argc, char **argv) {
    if (argc != 5) {
        fprintf(stderr, "usage: %s core nr_updates nr_updates_per_service area_bytes\n", argv[0]);
        return -1;
    }
    ...
    if (root_desc == MAP_FAILED) {
        cerr << "test_ctrl: tasvir_init failed" << std::endl;
        return -1;
    }
    ...
    */
    ...
    for (do_service = 1; do_service >= 0; do_service--)
        for (do_log = 1; do_log >= 0; do_log--) {
            ...
            time_ms[do_log * 2 + do_service] = experiment(d, count, iter);
        }
