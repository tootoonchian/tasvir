...
protected:
    ...
    virtual void ApplyGradient(const Datapoint &datapoint) {
        ...
        for (const auto &c : datapoint.GetCoordinates()) {
            const auto &mu = Mu(c);
            tasvir_log_write(&model->Data2D(c, 0, false), sizeof(double) * n_coords);
            for (int j = 0; j < n_coords; j++)
                model->Data2D(c, j, false) = (1 - mu) * model->Data2D(c, j, false) - Nu(c, j) + H(c, j);
        }
    }
    ...
    virtual void CatchUp(int index, int diff) {
        ...
        tasvir_log_write(&model->Data2D(index, 0, false), sizeof(double) * n_coords);
        for (int j = 0; j < n_coords; j++)
            model->Data2D(index, j, false) =
                pow(1 - mu, diff) * model->Data2D(index, j, false) - Nu(index, j) * geom_sum;
    }
